...
def process_dataset(
    ...
    for video_path, annotation_path in video_bar:
        ...
        try:
            ...
            num_frames = int(skeleton.shape[0])
            metadata.append(
                {
...
def main():
    parser = argparse.ArgumentParser(description="LE2I dataset processing.")
    parser.add_argument("--dataset-root", type=Path, default=Path("dataset/le2i"))
    parser.add_argument("--output-dir", type=Path, default=Path("dataset/le2i/pose_npy"))
    parser.add_argument("--metadata", type=Path, default=Path("dataset/le2i/pose_npy/metadata.json"))
